...
def calculate_similarity_matrix(distance_matrix):
    ...
    ones_matrix = np.ones((num_nodes, num_nodes))
    S = (max_distance * ones_matrix - distance_matrix) / max_distance
...
def euclidean_distance_matrix(features_matrix):
    ...
    for i in range(num_nodes):
        for j in range(num_nodes):
            if j>=i:
                M[i, j] = np.linalg.norm(features_matrix[i] - features_matrix[j])
    np.fill_diagonal(Q,np.diagonal(M))
    M = M+M.T-Q
